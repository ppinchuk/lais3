...
class AgentThread(Thread):
    def __init__(self, out_io, queue):
...
class AgentRunner:
    def __init__(self, command, agent_processes, thread, queue):
        self._agent_processes = agent_processes
        self._agent_thread = thread
        self._queue = queue
        self._command = command

    def run_agent(self, observation, configuration):
    ...
    def start_agent(self, observation, configuration):
        agent_path = (
            configuration["__raw_path__"]
        ...
        agent_process = Popen(
            self._command, stdin=PIPE, stdout=PIPE, stderr=PIPE, cwd=agent_cwd
        )
...
def command_agent(observation, configuration):
